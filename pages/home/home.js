...
Page({
  ...
  data: {
		...
		imgUrls: [],  //轮播
		msgList: [
        { url: "url", origin: "广州",end:'美国',type:"国际搬家",time:"12-6"},
        { url: "url", origin: "深圳",end:'澳大利亚',type:"国际搬家",time:"12-6"},
        { url: "url", origin: "河源",end:'广州',type:"国内搬家	",time:"12-6"},
		],	//实时订单
		msgList_two: [
			{name:'开玩笑共享搬家成功'},
			{name:'是固定共享搬家成功'},
  ...
  onReady: function() {
    ...
    app.util.request({
      ...
      success:res=>{
		  const data = res.data.data.message.slides
        console.log('res: ',res.data.data.message.slides);
		this.setData({
			imgUrls:data
		})
      }
